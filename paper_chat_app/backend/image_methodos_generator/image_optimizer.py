...
async def generate_and_save_image(
    ...
    prompt = f"""You are generating an 16:9 landscape academic infographic image. You MUST follow the provided Render Blueprint exactly.
    ...
    TEXT PRESERVATION RULES (MANDATORY):
    ...
    LEGEND RULE:
    - Legend must appear bottom-right.
    ...
    - Do NOT remove or shorten ANY label or bullet.
    - Every bullet must appear as a separate bullet.
    - Do NOT merge or omit any steps or substeps.
    - Do NOT rename anything.
    - All arrow labels must match exactly.
    """
...
async def criticize_image_with_logics(ai_client, request_dir: str, image_path: str):
    # critize the image with the ground_truth, return the a list of issues
    system_prompt = """
    You are an expert at analyzing academic infographic images and extracting all visible text content with high accuracy."""

    QUESTION_list = [
        "what is the missing step in the image? Answer:",
        "what is the missing arrow in the image? Answer:",
        "what is the incorrect order of steps in the image? Answer:",
        "what is the duplicated content in the image? Answer:",
        "what is the mislabeled content in the image? Answer:",
        "what is the overlapping content in the image? Answer:",
    ]
    ...
    Answer only the questions above, do not look for external information."""
    ...
    try:
        ...
        try:
            with open(os.path.join(request_dir, "issues.txt"), "w") as f:
                f.write(result_content)
            return {
                "issues": result_content
            }
        except json.JSONDecodeError as e:
            logger.error(f"Failed to parse JSON response: {str(e)}")
            logger.error(f"Response content: {result_content[:500]}")
            return {
...
async def criticize_image_with_render_text(ai_client, request_dir: str, image_path: str):
    ...
    system_prompt = """
    You are an expert at analyzing academic infographic images and extracting all visible text content with high accuracy.
    You are given an image. You need to extract information from the image and dicover the logic or layout issues in the image."""
    ...
    try:
        ...
        try:
            ...
            with open(os.path.join(request_dir, "issues.json"), "w") as f:
                json.dump(extracted_text, f, indent=2)
            return {
                "issues": extracted_text
            }
        except json.JSONDecodeError as e:
            logger.error(f"Failed to parse JSON response: {str(e)}")
            logger.error(f"Response content: {result_content[:500]}")
...
async def edit_image_with_issues(ai_client, request_dir: str, image_path: str):
    ...
    Only fix the issues. Do NOT change any other part of the image.
    ...
    try:
        ...
        logger.info(f"Image edited and saved to: {edited_image_path}")
        return {
