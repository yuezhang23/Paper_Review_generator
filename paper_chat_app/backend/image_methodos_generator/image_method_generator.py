...
from .image_optimizer import generate_and_save_image, criticize_image_with_render_text, regenerate_on_issues
...
async def _generate_interpretation(ai_client, retrieved_content: str) -> str:
    ...
    for _ in range(3):
        ...
        elapsed_time = time.time() - start_time
        logger.info(
            f"ai_client.chat.completions.create (interpretation {len(interpretations) + 1}) "
...
async def _generate_and_refine_image(
    ai_client,
    ...
    for i in range(max_retries):
        ...
        if criticize_image: 
            criticism = await criticize_image_with_render_text(ai_client, request_dir, image_path)
        elapsed_time = time.time() - iter_start
        logger.info(f"generate_and_save_image (iter {i + 1}) took {elapsed_time:.2f} seconds")

        if not regenerate_on_issues(criticism):
            break

    issues_path = os.path.join(request_dir, "issues.json")
    with open(issues_path, "w", encoding="utf-8") as f:
        json.dump(criticism, f, indent=2)

    return image_bytes, image_url, criticism


async def generate_summary_image(request: ImageGenerationRequest):
    function_start_time = time.time()
    try:
        # Step 1: Resolve methodology content from PDF
        pdf_path = None
        index = None
        ...
        # Step 2: Generate step-by-step interpretation using AI
        ...
        # Step 3: Generate whiteboard diagram prompt
        start_time = time.time()
        result = await generate_from_file(interpretation_path, request_dir)
        elapsed_time = time.time() - start_time
        whiteboard_prompt = result["layer3_render"]
        
        # Step 4: Generate and save image using the optimized function
        start_time = time.time()
        max_retries = 5
        criticism = []
        image_bytes = None
        ...
        # edited_image = await edit_image_with_issues(ai_client, request_dir, image_path)
