...
"""
    ...
    user_prompt = f"""Given the LOGIC LAYER and LAYOUT LAYER below, generate a Render Blueprint for a 16:9 academic methodology diagram.

Your output MUST include the following sections A~D in order:
...
D. VERBATIM TEXT TABLE
a complete list of EVERY text string that must appear in the image.
Explicitly state: any missing string = invalid render.

OUTPUT CONTRACT (STRICT):
- Do NOT include introductions, commentary, explanations, markdown, or apologies.
- Do NOT restate the user request.
- Do NOT add any section outside A–D.
...
Output must include all sections A–D. Any omission will be considered a failure.
"""
    ...
    for attempt in range(max_retries):
        try:
            ...
            if not content.startswith('A.') or not content.contains('D.'):
